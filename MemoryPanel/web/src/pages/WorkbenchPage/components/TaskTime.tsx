...
interface Entry { id: string; author: string; started: number; ended: number | null; seconds: number | null; note: string; kind: string }
...
export default function TaskTime({ taskId, currentUser }: { taskId: string; currentUser: string }) {
 ...
 return <section className="project-board-time">
  ...
  {entries.map(entry=><article className="project-board-time-entry" key={entry.id}>
   ...
   {entry.author===currentUser && entry.ended!==null && <button disabled={busy} onClick={()=>{if(window.confirm(t('time.removeConfirm')))void mutate('delete',{id:entry.id});}}>{t('board.activity.delete')}</button>}
  </article>)}
