...
function Preview() {
 ...
 return <main style={{padding:32}}><p style={{marginBottom:12,color:"#64748b"}}>Local preview · sample tasks · changes do not affect the live trial</p><div className="workbench-view-switch"><button onClick={()=>setView('board')}>Task board</button><button onClick={()=>setView('time')}>Timesheets</button><button onClick={()=>setView('loops')}>Loops</button></div>{view==='loops'?<Loops teamId="preview" currentUser="john" agents={[{id:'preview-agent',name:'Research assistant'}]} onOpenTask={()=>setView('board')}/>:view==='time'?<Timesheets teamId="preview"/>:<BoardView tasks={tasks} tasksLoading={false} selected={tasks.find(t => t.task_id === id) ?? null} onSelect={setId}
