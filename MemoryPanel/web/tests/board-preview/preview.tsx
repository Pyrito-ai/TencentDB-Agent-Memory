import Agenda from '../../src/pages/WorkbenchPage/components/Agenda';
import { addDays, localDay } from '../../src/services/task-schedule';
import Loops from '../../src/pages/WorkbenchPage/components/Loops';
...
function Preview() {
 const [tasks, setTasks] = useState<Task[]>(BOARD_STATUSES.map((status, i) => ({ task_id: `preview-${i}`, team_id: 'preview', creator_user_id: 'john', participants: [], title: titles[i], description: 'Keep the work clear and the next step easy to find. No agent required.', source_type: 'manual', source_url: '', linked_agents: [], status: status === 'done' ? 'completed' : 'running', created_at_ms: Date.now(), updated_at_ms: Date.now(), metadata_json: writeTaskBoard('{}', { status, priority: i === 2 ? 'high' : 'medium', assignee: i === 1 ? '' : 'john', plannedStart: i === 2 ? addDays(localDay(), -2) : '', dueDate: i === 0 ? addDays(localDay(), -1) : i === 1 ? localDay() : i === 2 ? addDays(localDay(), 4) : i === 3 ? addDays(localDay(), 12) : '' }) })));
 const [loopSelection,setLoopSelection]=useState<{id?:string;due?:string}>({});
 const [view,setView]=useState(new URLSearchParams(location.search).get('view') || 'board');
 const [id, setId] = useState<string | null>(null);
 return <main style={{padding:32}}><p style={{marginBottom:12,color:"#64748b"}}>Local preview · sample tasks · changes do not affect the live trial</p><div className="workbench-view-switch"><button onClick={()=>setView('today')}>Today</button><button onClick={()=>setView('upcoming')}>Upcoming</button><button onClick={()=>setView('board')}>Task board</button><button onClick={()=>setView('time')}>Timesheets</button><button onClick={()=>setView('loops')}>Loops</button></div>{view==='today'||view==='upcoming'?<Agenda view={view} teamId="preview" tasks={tasks} loading={false} currentUser="john" onOpenTask={id=>{setId(id);setView('board');}} onOpenLoops={(id,due)=>{setLoopSelection({id,due});setView('loops');}}/>:view==='loops'?<Loops initialLoop={loopSelection.id} initialDue={loopSelection.due} members={team.members} teamId="preview" currentUser="john" agents={[{id:'preview-agent',name:'Research assistant'}]} onOpenTask={()=>setView('board')}/>:view==='time'?<Timesheets teamId="preview"/>:<BoardView tasks={tasks} tasksLoading={false} selected={tasks.find(t => t.task_id === id) ?? null} onSelect={setId}
