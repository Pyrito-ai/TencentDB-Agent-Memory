...
## Loops — manual recurring work

Workbench → Loops extends the persistent Panel data model with `loops`, `loop_occurrences`, `loop_time`, `loop_settings`, and `loop_audit` tables in `time.sqlite`. Every Loop belongs to a team and an active Project; there is no area or separate client dimension. Team members create Loops; the creator or team admin/owner can edit/archive/restore. Archive preserves occurrence history. Project and contributor filters are available. Project/client tenancy follows active team membership on every route.

Targets are 1–100 accepted completions per day, Monday-based week, or calendar month. These are flexible period targets, not automated schedules or specific-day reminders. A team admin can select an IANA timezone before the first Loop is created (default UTC). Timezone then stays fixed; target/frequency cannot change after an occurrence exists. This prevents edits from rewriting historical streaks. Current streak includes an ongoing incomplete period's immediately preceding streak; best streak counts consecutive periods meeting the full target. Acceptance time determines the completion period, including for late work. Completion history is append-oriented with audited undo; no retroactive/backdated completion interface is included.

Starting an occurrence creates a linked Core task, associates it with the Project, and snapshots the brief/project/contributor. Human work starts In progress; an agent handoff starts Ready and links the selected eligible agent. Team-visible active agents or agents owned by the caller can be linked; cross-team agents are rejected. Handoffs are manual: copy the brief with task, agent, project, and occurrence identifiers into the agent client. No runtime is launched and no model call is made. Sessions recorded using that task ID continue to appear in its existing participation view. Optional result URLs attach work evidence; the service does not fetch them. A human contributor must explicitly accept the occurrence. Core task lifecycle remains independently editable on the board.

Each contributor starts and accepts their own occurrences, contributing to a shared Loop target. Other teammates can see the history, but cannot claim another contributor's occurrence. Human time can be logged with the existing timer/manual UI and optionally linked at completion. Only the contributor's stopped entries on that task are eligible; linking does not duplicate time. Linked entries cannot be removed until the completion is undone. Approved/paid linked time prevents undo; Timesheets approval/payment rules remain authoritative.
